-- Add real-time support for all tables
...
-- Update RLS policies for approval system
CREATE POLICY "Admins can view all profiles including pending approval"
ON public.profiles
FOR SELECT
USING (
  is_user_admin() OR 
  (auth.uid() = id) OR 
  (department_id IS NOT NULL AND department_id = get_current_user_department() AND approval_status = 'approved')
);
